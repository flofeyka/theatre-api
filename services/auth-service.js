...
class authService {
    async signIn({email, password}) {
        ...
        const {refreshToken, accessToken} = tokenService.generateTokens({id: userFound.id, fullName: userFound.fullName, email: userFound.email});
    ...
    async signUp({
        ...
        const userCreated = await userService.create({ fullName, phoneNumber, password: passwordHash, email, birth: new Date(birth) });
        const { accessToken, refreshToken } = tokenService.generateTokens({ id: userCreated.id, fullName: userCreated.fullName, email: userCreated.email });
    ...
    async refresh(currentRefreshToken) {
        ...
        const {accessToken, refreshToken} = tokenService.generateTokens({id: tokenData.user.id, fullName: tokenData.user.fullname, email: tokenData.user.email});
